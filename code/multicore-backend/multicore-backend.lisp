...
(defmethod backend-compute
    ((backend multicore-backend)
     (lazy-arrays list))
  (let ((request (petalisp.core:make-request backend lazy-arrays)))
    (backend-schedule backend request)
    (wait request)
...
(defmethod request-finish ((request request))
  ...
  (bordeaux-threads:with-recursive-lock-held (*lazy-array-lock*)
    (loop for lazy-array in (request-lazy-arrays request)
          ...
          do (when (typep delayed-action 'delayed-wait)
               (setf (delayed-wait-delayed-action delayed-action)
                     delayed-array))
