      ...
      (let ((type-specifier-1
        ...
        (flet ((matches (type)
          ...
          (let ((set-1 (remove-if-not (matches type-specifier-1) *test-numbers*))
                ...
                (union (remove-if-not (matches union-type-specifier) *test-numbers*)))
            (is (<= (length (cl:set-difference (cl:union set-1 set-2) union))
                    (length (cl:set-difference union (cl:union set-1 set-2)))))))))))
